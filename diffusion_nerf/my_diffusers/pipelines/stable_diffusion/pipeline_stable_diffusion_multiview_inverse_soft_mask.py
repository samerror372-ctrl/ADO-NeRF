...
class StableDiffusionMultiViewPipeline(
    ...
    def __call__(    #####  Denoise
        ...
        if (render_latents is not None) and (mask_batch is not None):
           ...
           # ---- 對齊到 (h', w') ----
            mask_pooled = F.adaptive_max_pool2d(mask_1c, output_size=(h_small, w_small))  # [F,1,h',w']
            base = mask_pooled.clamp(0, 1)  # 保留連續值，不做二值化

            # ---- 形態學「灰階膨脹」：用 max-pool 即可（會擴張高值區域，仍保持 [0,1] 連續）----
            def gray_dilate(x, k=5, iters=1):
                pad = k // 2
                out = x
            ...
            # 三個不同擴張半徑（依然是 soft mask）
            mask_35 = base                             # 原始
            mask_25 = gray_dilate(base, k=5, iters=1)  # 輕度擴張
            mask_15 = gray_dilate(base, k=5, iters=2)  # 更大擴張

            # （選配）再做一點 feather/平滑，避免邊界太硬
            def feather(x, k=5, iters=1):
                pad = k // 2
                y = x
                for _ in range(iters):
                    y = F.avg_pool2d(y, kernel_size=k, stride=1, padding=pad)
                return y

            mask_35 = feather(mask_35, k=5, iters=1).clamp(0, 1)
            mask_25 = feather(mask_25, k=5, iters=1).clamp(0, 1)
            mask_15 = feather(mask_15, k=5, iters=1).clamp(0, 1)
            ...
            for i, t in enumerate(timesteps):   ### 
                ...
                if (render_latents is not None) and (i in trigger_points):
                    ...
                    if not use_mask_blend:
                        ...
                        # === 準備 target 區段 ===
                        tgt_idx = torch.arange(cond_num, F, device=device)  # 從 cond_num 到最後
                        ...
                        # === noise 注入到 render latent ===
                        if i < L - 1:
                            t_prev = timesteps[i + 1] ### !!!重要 前一步強度(t-1!!!!!)
                        else:
                            t_prev = timesteps[i]

                        if hasattr(scheduler, "alphas_cumprod"): ##THIS!
                            #breakpoint()
                        ...
                        # === Mask blend ===
                        #mask_tgt = mask_lat[:, tgt_idx].to(device=device, dtype=dtype)
                        ...
                        # Noise Resampling
                        # ===== k 次 noise resampling（整個 target 區段；用 t_prev 強度 re-noise，然後以 t_prev 再 denoise 一步）=====
                        # 白(1) → 原本 latent；黑(0) → rdl_noisy
                        # ===== k 次 noise resampling（保持時間步對齊：用「當前步 t」re-noise，再以 t denoise 回 x_{t-1}）=====
                        
                        # === Noise resampling（用「同一個時間步」重抽樣）===

                        # 取 ᾱ_t 與 ᾱ_{t_prev}（CPU 索引 → 搬 device）
                        ...
                        extra_resample_steps = 3 #0 #3
                        for _ in range(extra_resample_steps):
                            # === (A) 先用 s = t_prev 從目前 latents 推出 x0（只算 target 區段）===
                            ...
                            # 對齊維度 → 取 target 幀的預測
                            eps_full = noise_pred_prev
                            if eps_full.dim() == 4:            # [F, C, H, W]
                                eps_full = eps_full.unsqueeze(0)  # → [1, F, C, H, W]
                            x_s_region  = latents[:, tgt_idx]     # [B, replace_n, 4, H, W]（此刻是 x_{t-1}）
                            eps_region  = eps_full[:, tgt_idx]    # [B, replace_n, 4, H, W]
                            # 若通道數不一致，裁到 latent 的通道數
                            if eps_region.shape[2] != x_s_region.shape[2]:
                                eps_region = eps_region[:, :, :x_s_region.shape[2]]

                            # 用 s = t_prev 的公式推 x0（依 prediction_type）
                            ...
                            # === (B) 用「當前步 t」把 x0 升噪到 x_t（只限 target 區段）===
                            eps_t      = torch.randn_like(x0_region)
                            x_t_region = alpha_t * x0_region + sigma_t * eps_t

                            # === (C) 以「同一個 t」跑 UNet + step，把 x_t → x_{t-1}，只回寫 target 區段 ===
                            ...
                            # 只更新 target 區段，其他區域（尤其 cond 幀）不動
                            latents[:, tgt_idx] = latents_step[:, tgt_idx]
                            latents[:, :cond_num] = image_latents[:, :cond_num]   # 鎖回條件幀
