        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):   ### 
                ...
                if i > int(0.7 * len(timesteps)):
                    ...
                    # 加入 DDIM 噪聲
                    noise = torch.randn_like(resize_image_latents)
                    replace_with = sqrt_alpha_bar * resize_image_latents + sqrt_one_minus_alpha_bar * noise
                """
                # noise -> t-20
                ...
                if i < len(timesteps) - 2 and i > int(0.7 * len(timesteps)): ## len(timesteps = 50)
     
                    # 取得 t-1 的 timestep
                    t_prev = timesteps[i + 1] ## 261
                    ### t_prev 281
                    ### i = 36 
                    
                    #breakpoint()
                    # 對應到 t-20 的 ᾱ
                    alpha_bar_prev = scheduler.alphas_cumprod[t_prev]  ##t-20 強度的noise
                    sqrt_alpha_bar_prev = alpha_bar_prev.sqrt()
                    sqrt_one_minus_alpha_bar_prev = (1 - alpha_bar_prev).sqrt()

                    # 加入 t-20 強度的噪聲
                    noise = torch.randn_like(resize_image_latents)
                    replace_with = sqrt_alpha_bar_prev * resize_image_latents + sqrt_one_minus_alpha_bar_prev * noise

                
                
                """
                
                if i > int(0.7 * len(timesteps)):  # 後 30% 步驟：貼 clean latent（不加 noise）
                    replace_with = resize_image_latents
                    #replace_with = torch.zeros_like(resize_image_latents)
                
                """
                
                # 若需要貼入 latent
                if replace_with is not None: #非中間區域
                    top = (H - resize_H) // 2
                    left = (W - resize_W) // 2
                    for j in range(half):
                        latents[:, half + j, :, top:top+resize_H, left:left+resize_W] = replace_with[j]
