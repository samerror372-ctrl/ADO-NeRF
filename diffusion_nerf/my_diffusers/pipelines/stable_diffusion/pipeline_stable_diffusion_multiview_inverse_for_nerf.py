...
class StableDiffusionMultiViewPipeline(
    ...
    def __call__(
        ...
        render_batch = pack_render_imgs_to_tensor(self.render_imgs, device=device, target_hw=(height, width))
        mask_batch = pack_render_imgs_to_tensor(self.mask_imgs, device=device, target_hw=(height, width))

        if render_batch is not None:
            render_latents = self._encode_vae_image(image=render_batch, generator=generator)
        ...
        if (render_latents is not None) and (mask_batch is not None):
            ...
            mask_pooled = F.adaptive_max_pool2d(mask_1c, output_size=(h_small, w_small))
            base = (mask_pooled >= 0.5).float()
            ...
            self.mask_latents_25 = dilate(base, k=5, iters=1).unsqueeze(0)
            self.mask_latents_15 = dilate(base, k=5, iters=2).unsqueeze(0)
        else:
        ...
        if kwargs['config'].model_cfg.get("enable_depth", False):
            if kwargs['config'].model_cfg.get("priors3d", False):
                ...
                depth = einops.rearrange(kwargs["depth"], "(b f) c h w -> b f c h w", b=batch_size, f=nframe).to(device=device)
                depth[:, cond_num:] = 0
                    ...
                    if mask_lat is not None:
                        ...
                        for _ in range(3):
                            ...
                            noise_pred_prev = self.unet(
                                latent_in_prev, t_prev_tensor, add_inputs=add_inputs,
                            ...
                            if pred_type == "epsilon":
                                x0_region = (x_s_region - (1.0 - alpha_bar_prev).sqrt() * eps_region) / alpha_bar_prev.sqrt()
                            elif pred_type == "v_prediction":
                                x0_region = alpha_bar_prev.sqrt() * x_s_region - (1.0 - alpha_bar_prev).sqrt() * eps_region
                            else:
