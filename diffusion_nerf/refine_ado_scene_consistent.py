...
def main():
    ...
    output_dir = os.environ.get("ADO_REFINE_OUTPUT_DIR", os.path.join(DIFFUSION_ROOT, f"ado_result_scene{paste_suffix}"))
    debug_dir = os.environ.get("ADO_REFINE_DEBUG_DIR", os.path.join(DIFFUSION_ROOT, f"ado_debug_scene{paste_suffix}"))
    ...
    print(f"[ADO Scene] input_data_dir = {input_data_dir}")
    print(f"[ADO Scene] output_dir = {output_dir}")
    print(f"[ADO Scene] diffusion_steps = {diffusion_steps}, guidance_scale = {guidance_scale}")
    print(f"[ADO Scene] repair_mask_threshold = {repair_mask_threshold}")
    if max_files > 0:
        print(f"[ADO Scene] max_files = {max_files}")
