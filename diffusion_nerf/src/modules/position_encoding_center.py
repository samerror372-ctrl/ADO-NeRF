    ...
    if colors is not None or latents is not None: #[1, 27, 3, 264, 480]
        if vae is not None:
            ...
            if return_pixel:
                ...
                #coords = torch.cat([imgs_blank, alpha_blank], dim=1)  # [B*F, 4, H, W]
                coords = torch.cat([imgs, coord_mask], dim=1)

                
                ENABLE_PASTE = bool(config.model_cfg.get("enable_pixel_prior_paste", True))

                if tar_idx_to_cond_idx is not None and ENABLE_PASTE:
                    
                    B = imgs.shape[0] // nframe

                    import torch.nn.functional as F_torch  # 改別名
                    ...
                    scale = zoom_scale  # 貼上的 ref 圖相對尺寸
                    B, nF, C, H, W = bf_imgs.shape  # 不要用 F

                    for j, src in enumerate(tar_idx_to_cond_idx):
                        if src >= cond_num:
                            src = 0
                        tgt = cond_num + j
                        ref = bf_imgs[:, src]  # [B, 3, H, W]

                        # 1) 縮小一半
                        patch_h = max(1, int(round(H * scale)))
                        patch_w = max(1, int(round(W * scale)))
                        ref_patch = F_torch.interpolate(
                            ref, size=(patch_h, patch_w), mode='bilinear', align_corners=False
                        )  # [B, 3, patch_h, patch_w]
                        ...
                        # 3) 貼到 target 的中心，只覆蓋該區塊
                        bf_imgs[:, tgt, :, y0:y1, x0:x1] = ref_patch

                        # 4) alpha 中心區塊設 1（可見且確定）
