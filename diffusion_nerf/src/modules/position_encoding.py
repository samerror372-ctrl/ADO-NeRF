...
def get_3d_priors(config, depth, K, w2c, cond_num, nframe, device, contract=True,
                  colors=None, latents=None, vae=None, prior_type="3dpe", tar_idx_batch=None):
    ...
    if prior_type == "3dpe":
        ...
        coords_3dpe = global_position_encoding_3d(config, depth, K, w2c, cond_num, nframe, device,
                                                  pe_scale=1 / 8, embed_dim=192, contract=contract, tar_idx_batch=tar_idx_batch)  # [27, 193, 33, 60]
        coords_pixel = global_position_encoding_3d(config, depth, K, w2c, cond_num, nframe, device,
                                                   pe_scale=1.0, embed_dim=4, contract=True, colors=colors, vae=vae,
                                                   return_pixel=True, tar_idx_batch=tar_idx_batch) # [27, 4, 264, 480]  #cond_num 20 #nframe 27
...
def global_position_encoding_3d(config, depth, K, w2c, cond_num, nframe, device,
                                ...
                                vae=None, return_pixel=False, tar_idx_batch=None):
    ...
    if colors is not None or latents is not None: #[1, 27, 3, 264, 480]
        if vae is not None:
            ...
            if return_pixel:
                ...
                coords = torch.cat([imgs, coord_mask], dim=1)  # [B*F, 4, H, W]
                
                if tar_idx_to_cond_idx is not None:
                    ...
                    for j, src in enumerate(tar_idx_to_cond_idx):
                        assert 0 <= src < cond_num, f"src={src} 超出 ref 範圍"
                        # 直接把第 j 個 tar（局部 idx = cond_num + j）的 RGB 換成第 src 個 ref
                        bf_imgs[:, cond_num + j] = bf_imgs[:, src]
                        # 如果要完全覆蓋，把 alpha 設 1；若只想保留 warp 可見區就別動：
                        # bf_alpha[:, cond_num + j] = 1.0
