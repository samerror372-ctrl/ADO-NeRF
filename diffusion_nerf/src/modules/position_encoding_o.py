...
def get_3d_priors(config, depth, K, w2c, cond_num, nframe, device, contract=True,
    ...
    if prior_type == "3dpe":
        coords = global_position_encoding_3d(config, depth, K, w2c, cond_num, nframe, device,
                                             pe_scale=1 / 8, embed_dim=192, contract=contract, zoom_scale=zoom_scale)

    elif prior_type == "latent":   ####color?
        #breakpoint()
        coords = global_position_encoding_3d(config, depth, K, w2c, cond_num, nframe, device,
                                             pe_scale=1.0, embed_dim=4, contract=True, colors=colors, vae=vae, zoom_scale=zoom_scale)
        ...
        coords = global_position_encoding_3d(config, depth, K, w2c, cond_num, nframe, device,
                                             ...
                                             vae=vae, return_pixel=True, tar_idx_batch=tar_idx_batch, zoom_scale=zoom_scale)
    elif prior_type == "3dpe+latent":
        coords_3dpe = global_position_encoding_3d(config, depth, K, w2c, cond_num, nframe, device,
                                                  pe_scale=1 / 8, embed_dim=192, contract=contract, tar_idx_batch=tar_idx_batch, zoom_scale=zoom_scale)
        coords_latent = global_position_encoding_3d(config, depth, K, w2c, cond_num, nframe, device,
                                                    pe_scale=1.0, embed_dim=4, contract=True, colors=colors, vae=vae, tar_idx_batch=tar_idx_batch, zoom_scale=zoom_scale)
        ...
        coords = global_position_encoding_3d(config, depth, K, w2c, cond_num, nframe, device,
                                             ...
                                             return_pixel=True, tar_idx_batch=tar_idx_batch, zoom_scale=zoom_scale)
        print("pixel!")
        #breakpoint()
        ...
        coords_3dpe = global_position_encoding_3d(config, depth, K, w2c, cond_num, nframe, device,
                                                  pe_scale=1.0, embed_dim=48, contract=contract, tar_idx_batch=tar_idx_batch, zoom_scale=zoom_scale)
        coords_pixel = global_position_encoding_3d(config, depth, K, w2c, cond_num, nframe, device,
                                                   pe_scale=1.0, embed_dim=4, contract=True, colors=colors, vae=vae,
                                                   return_pixel=True, tar_idx_batch=tar_idx_batch, zoom_scale=zoom_scale)
        return [coords_3dpe, coords_pixel]
...
def global_position_encoding_3d(config, depth, K, w2c, cond_num, nframe, device,
    ...
    tar_num = nframe - cond_num
    btar = (depth.shape[0] // nframe) * tar_num  # = b*tar_num
    # 當前影像尺寸
    bc, _, h, w = depth.shape
    # N = b*tar_num*hw（你後面把東西都攤平成這個長度）
    N = btar * h * w

    # 若沒提供對應，預設第 k 個 target 只接受第 k 個 cond（循環對齊）
    if tar_idx_batch is None:
        # tar2cond: [b, tar]，值域在 [0, cond_num)
        tar2cond = torch.arange(tar_num, device=device) % cond_num
        tar2cond = tar2cond.unsqueeze(0).expand(b, -1)  # [b, tar]
    else:
        tar2cond = torch.as_tensor(tar_idx_batch, device=device).long()  # [b, tar]

    # 展開到每個像素（與你 later 的 flatten 對齊）
    # tar2cond_flat: [N]，表示「這個 flattened 位置應該只接受哪個 cond」
    #tar2cond_flat = tar2cond.repeat_interleave(h * w, dim=1).reshape(-1)  # [N]
    hw = h * w

    def normalize_tar2cond(tar2cond, b, tar_num, device):
        """
        將 tar2cond 正規化為 [b, tar]，元素範圍應該在 [0, cond_num)
        可接受形狀：
        - [tar]：會複製到每個 batch，變成 [b, tar]
        - [b, tar]：直接使用
        - [b*tar]：reshape 成 [b, tar]
        """
        tc = tar2cond.to(device).long()
        if tc.dim() == 1:
            if tc.numel() == tar_num:
                tc = tc.unsqueeze(0).expand(b, -1)     # [b, tar]
            elif tc.numel() == b * tar_num:
                tc = tc.view(b, tar_num)               # [b, tar]
            else:
                raise ValueError(f"tar2cond shape incompatible: {tuple(tc.shape)}; expect {tar_num} or {b*tar_num} elements")
        elif tc.dim() == 2:
            if tc.shape != (b, tar_num):
                raise ValueError(f"tar2cond expected shape [b, tar]={b, tar_num}, got {tuple(tc.shape)}")
        else:
            raise ValueError(f"tar2cond dim must be 1 or 2, got dim={tc.dim()}")
        return tc

    tar2cond_bt = normalize_tar2cond(tar2cond, b, tar_num, device).reshape(-1)   # [b*tar]
    # 展開到每個像素，得到 [N = b*tar*hw]
    tar2cond_flat = torch.repeat_interleave(tar2cond_bt, repeats=hw)             # [N]   
    ...
    for i in range(cond_num):
        ...
        proj_mask = (1 - torch.clamp(x_mask + y_mask + z_mask, 0, 1)).float()  # [N]

        # ★ 只允許「被指定該 cond 的 target 像素」通過
        allowed = (tar2cond_flat == i)              # [N]，True 才能被第 i 個 cond 寫入
        proj_mask *= allowed.float()                # 套用一對一配對限制

        proj_index_ = proj_index[:, i]  # [N]
