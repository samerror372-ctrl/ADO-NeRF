...
from utils import _resize_pil_image, process_prompt, load_16big_png_depth, _resize_np_image


class GL3DDataset(Dataset):
    def __init__(self, dataset_root, data_list, nframe, height, width,
                 rank=0, mode="train", min_interval=1, max_interval=4, **kwargs):
        super(GL3DDataset, self).__init__()
        ...
        self.global_seq_dict = pickle.load(open("./data/gl3d/global_seq_dict.pkl", "rb"))
        self.total_num = 0
        with open(data_list, "r") as f:
            for line in tqdm(f, desc=f"loading global gl3d {mode} sequences...", disable=rank != 0):
    ...
    def reset_dataset(self, mode="train", rank=0, **kwargs):
        ...
        for i, class_name in enumerate(class_names):
            ...
            seq_indices = [f"{class_name}/undist_images/{s}" for s in seq_indices]
    ...
    def __getitem__(self, index):
        try:
            ...
            print("Error image from", f"{self.dataset_root}/{self.data_list_per_epoch[index]}")
        ...
        # all gl3d images should be 1000*1000, but maybe some images are rectangular
        intrinsic = self.center_crop_intrinsic(intrinsic, h0=new_h, w0=new_w, h1=height, w1=width)

        caption_path = f"{self.caption}/{seq_name.split('/')[0]}.txt"
