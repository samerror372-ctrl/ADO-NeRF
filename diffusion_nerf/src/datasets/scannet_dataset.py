...
from utils import _resize_pil_image, load_16big_png_depth, _resize_np_image, process_prompt
...
class ScanNetDataset(Dataset):
    ...
    def load_camera(self, fname):
        ...
        if self.camera_longest_side is not None:
            ...
            camera_scale_path = camera_scale_path.replace("/dslr/", "/")
    ...
    def __getitem__(self, index):
        ...
        image = _resize_pil_image(image, edge_size=min(height, width), longest=False,
                                  specific_wh=specific_wh, force_resize=True)
        if depth is not None:
            depth = _resize_np_image(depth, edge_size=min(height, width), longest=False,
                                     interpolation="nearest", specific_wh=specific_wh, force_resize=True)
        ...
        # we always use center crop for DL3DV to make intrinsic process easy
