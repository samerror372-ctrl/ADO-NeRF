...
from utils import _resize_pil_image, _seq_name_to_seed, _resize_np_image, process_prompt, load_16big_png_depth
...
class CO3Dv2Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        mask = Image.open(f"{self.dataset_root}/{class_seq_name}/masks/{filename.replace('.jpg', '.png')}").convert("L")
        ...
        mask = np.array(mask) / 255  # mask:[0~1]
        mask[mask > 0.5] = 1
        mask[mask <= 0.5] = 0
