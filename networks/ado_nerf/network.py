...
class Network(nn.Module):
    def __init__(self, 
        ...
        self.min_num_samples = getattr(config.nerf, 'min_num_samples', 1)
    ...
    def forward(self,
        ...
        # [NEW] 获取 Curriculum Learning 的 step_ratio 和 epoch_ratio
        step_ratio = batch.get('step_ratio', 1.0)
        epoch_ratio = batch.get('epoch_ratio', step_ratio) # 优先获取准确的 epoch 比例，否则退化使用 step_ratio
        ...
        # Estimate depth for TARGET view (常规操作)
        ...
        if epoch_ratio >= 0.05:
            ...
            # ⚠️ 极其重要：必须使用 torch.no_grad()，源深度仅用作物理标尺，不参与梯度回传，防止显存爆炸
            with torch.no_grad():
                for v in range(num_views):
                    # 依次将每个源相机伪装成“目标相机”传入 MVS 架构
                    ref_exts = src_exts[:, v]
                    ref_ints = src_ints[:, v]
                    
                    v_depths, v_depth_ranges, _, _, _ = self.depth_net(
                        src_images, ms_feats, src_exts, src_ints, ref_exts, ref_ints, near_far
                    )
                    # 提取该源视角最高分辨率的预测深度
                    src_depths_list.append(v_depths[-1]) 
                    src_depth_ranges_list.append(v_depth_ranges[-1])
            
            # 堆叠成 (B, num_views, H', W')
            src_mvs_depths = torch.stack(src_depths_list, dim=1) 
            src_mvs_depth_ranges = torch.stack(src_depth_ranges_list, dim=1)
            
            # 确保源深度图分辨率与 bundle spatial size 严格对齐
            ...
            if src_mvs_depths.shape[-2:] != (H_bundle, W_bundle):
                src_mvs_depths = F.interpolate(
                    ...
                    mode='nearest' # 使用 nearest 防止边缘过度平滑导致伪遮挡
                ).view(B, num_views, H_bundle, W_bundle)
                src_mvs_depth_ranges = F.interpolate(
                    src_mvs_depth_ranges.flatten(0, 1),
                    size=(H_bundle, W_bundle),
                    mode='nearest'
                ).view(B, num_views, 2, H_bundle, W_bundle)
        # =========================================================================
        ...
        rays_xyz, uvd, z_vals, ball_radii, indices, samples_per_batch, samples_per_bundle = \
            self.sampler.sample(depth_range, vol_range, self.b_size, self.max_num_samples, self.inv_depth, self.is_adaptive, self.min_num_samples)
        ...
        # [MODIFIED] 将计算好的 src_mvs_depths 传给 sampler.encode 进行遮挡剔除！
        # 如果 epoch_ratio < 0.05，传入的是 None，底层代码将直接跳过遮挡判定。
        rgbs_feat_rgb_dir, vox_feat, vis_weights = \
            self.sampler.encode(src_images, img_feat_rgb, feat_volume, rays_xyz, uvd, ball_radii, 
                                ...
                                src_mvs_depths=src_mvs_depths,
                                src_mvs_depth_ranges=src_mvs_depth_ranges,
        ...
        ret = {'rgb': img, 
               ...
               'src_mvs_depths': src_mvs_depths,
               'src_mvs_depth_ranges': src_mvs_depth_ranges} # <-- 新增
