...
class Network(nn.Module):
    ...
    def forward(self,
        ...
        # 获取 Curriculum Learning 的 step_ratio 和 epoch_ratio
        ...
        mvs_depth = mvs_depths[-1]  # 原始的 Target MVS 深度

        # =========================================================================
        # [核心机制] 动态预测源视角 (Source Views) 深度
        # =========================================================================
        src_mvs_depths = None
        ...
        # 👑 [绝妙解耦第一步] 送入 encode 的是包含近平面塌陷 BUG 的原始 MVS 深度
        # 这会继续保护背景不产生悬浮伪影
        rgbs_feat_rgb_dir, vox_feat, vis_weights = \
            self.sampler.encode(src_images, img_feat_rgb, feat_volume, rays_xyz, uvd, ball_radii, 
                                ...
                                src_mvs_depths=src_mvs_depths, step_ratio=step_ratio)
        ...
        # Merge coarse and fine RGB features, and reweight if required
        img = rgb_c + rgb_f  
        if self.reweighting:
            img = 0.5 * (img + rgb_f)

        # =========================================================================
        # 👑 [绝妙解耦第二步] 为外部 eval.py 彻底修复 MVS 深度 (目标视角 + 源视角)
        # 物理原理：将由于匹配失败塌陷到近平面(Near)或变成NaN的像素，全部推远到远平面(Far)
        # =========================================================================
        repaired_mvs_depth = mvs_depth.clone()
        repaired_src_mvs_depths = src_mvs_depths.clone() if src_mvs_depths is not None else None
        
        for b_idx in range(B):
            near_bound = near_far[b_idx, 0].item()
            far_bound = near_far[b_idx, 1].item()
            epsilon = 1e-3  # 容差，防止浮点数精度问题
            
            # 1. 修复目标视角 MVS (供 eval.py 做大范围背景拦截)
            is_nan_inf = torch.isnan(repaired_mvs_depth[b_idx]) | torch.isinf(repaired_mvs_depth[b_idx])
            is_near_collapse = repaired_mvs_depth[b_idx] <= (near_bound + epsilon)
            invalid_mask = is_nan_inf | is_near_collapse
            repaired_mvs_depth[b_idx][invalid_mask] = far_bound

            # 2. 修复源视角 MVS (保持输出数据的几何纯净性)
            if repaired_src_mvs_depths is not None:
                is_nan_inf_src = torch.isnan(repaired_src_mvs_depths[b_idx]) | torch.isinf(repaired_src_mvs_depths[b_idx])
                is_near_collapse_src = repaired_src_mvs_depths[b_idx] <= (near_bound + epsilon)
                invalid_mask_src = is_nan_inf_src | is_near_collapse_src
                repaired_src_mvs_depths[b_idx][invalid_mask_src] = far_bound
        # =========================================================================

        # 🚨 [输出打包] 将修复后的 MVS 深度数据抛出，供 eval.py 获取
        ret = {'rgb': img, 
               'nerf_depth': nerf_depth, 
               'mvs_depth': repaired_mvs_depth,          # 已修复！不再是近平面墙
               'opacity': nerf_opacity,
               'src_mvs_depths': repaired_src_mvs_depths} # 已修复！
