...
class EnerfBatchSampler(Sampler):
    def __init__(self, sampler, batch_size, drop_last, sampler_meta):
        ...
        self.render_scale = sampler_meta.render_scale
        self.render_prob = sampler_meta.scale_prob
        if cfg.fix_random:
            random.seed(0)
    ...
    def __iter__(self):
        ...
        render_scale = np.random.choice(self.render_scale, 1, p=self.render_prob)
        for idx in self.sampler:
            batch.append((idx, input_views_num.item(), render_scale.item()))
